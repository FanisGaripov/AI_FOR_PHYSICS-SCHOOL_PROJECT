...
def EGE_catalog():
    raw_catalog = get_catalog(True)
    processed_catalog = process_catalog(raw_catalog)
    return render_template('ege_catalog.html', catalog=processed_catalog)


@app.route('/oge', methods=['GET', 'POST'])
def OGE_catalog():
    raw_catalog = get_catalog(False)
    processed_catalog = process_catalog(raw_catalog)
    return render_template('oge_catalog.html', catalog=processed_catalog)
...
@app.route('/ege/<int:category_id>')
def EGE_zadaniya(category_id):
    return render_template('EGE.html', zadaniya=get_category_by_id(category_id, True))


@app.route('/oge/<int:category_id>')
def OGE_zadaniya(category_id):
    return render_template('OGE.html', zadaniya=get_category_by_id(category_id, False))
